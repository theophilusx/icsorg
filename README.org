...
*Caveats*: This script has not been heavily tested and has only been run on
 ...
 doing quite effectively. Also note that I'm currently using this project as a test case
 for experimenting with LLMs and using tools like [[https://opencode.ai][OpenCode]]. 
 
* Installation

A prerequisite for running this script is =node= and =npm=. Node.js v18 or later
is required.
...
The script includes comprehensive debugging functionality. When you run the script
with the =-d= flag, it will output detailed debug messages showing:
...
The debug output is invaluable for troubleshooting issues or understanding how the
script processes your calendar data.
...
The project includes a comprehensive test suite with 80+ tests covering all core
functionality:
...
See the [[https://github.com/theophilusx/icsorg/releases][releases page]] for a changelog.
...
For developers and contributors, see =AGENTS.md= for coding standards, conventions,
